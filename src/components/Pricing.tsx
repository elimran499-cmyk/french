...
const PACK_COLORS: Record<TierId, { base: PackHue; deep: PackHue }[]> = {
  basic: [
    { base: { from: '#6aa8f0', to: '#2f7fd4', ink: '#1263b4' },
      deep: { from: '#2f7fd4', to: '#0b4fa0', ink: '#0b4fa0' } },
    { base: { from: '#2f7fd4', to: '#0055a4', ink: '#0055a4' },
      deep: { from: '#0b5fbe', to: '#00337a', ink: '#00337a' } },
    { base: { from: '#0b5fbe', to: '#00337a', ink: '#00337a' },
      deep: { from: '#00408f', to: '#001f4d', ink: '#001f4d' } }
  ],
  vip: [
    { base: { from: '#ff8a80', to: '#f0554a', ink: '#d2382c' },
      deep: { from: '#f0554a', to: '#c62828', ink: '#b0231c' } },
    { base: { from: '#f0554a', to: '#d32f24', ink: '#b91c13' },
      deep: { from: '#dd3a2e', to: '#9c1710', ink: '#9c1710' } },
    { base: { from: '#d32f24', to: '#9c1710', ink: '#9c1710' },
      deep: { from: '#b0231c', to: '#6b0d08', ink: '#6b0d08' } }
  ]
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-8 sm:mb-10">
          ...
          <p className="hidden sm:block text-slate-400 text-sm sm:text-base">
            Commandez sur WhatsApp. Vos accès sont prêts en 5 minutes.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-8 pt-6 sm:pt-0 items-stretch">
          {DURATION_PACKS.map((pack, i) => {
            ...
            const orderText =
              `Bonjour ! Je souhaite commander le pack ${activeTier.name} ${pack.label} ` +
              `pour ${devices} ${devices === 1 ? 'écran' : 'écrans'} (${formatPrice(price)}). ` +
              `Merci de m'envoyer les modalités de paiement et d'activation.`;
              ...
              >
                ...
                <div className="relative px-6 sm:px-8 pt-6 sm:pt-8">
                  ...
                  <div className="text-center">
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
                      ...
                      <span className="px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-[11px] font-semibold text-slate-300 inline-flex items-center gap-1.5">
                        <Monitor className="w-3.5 h-3.5" />
                        {devices} {devices === 1 ? 'écran' : 'écrans'}
                      </span>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <span>{isVip ? 'Passer VIP' : 'Commander'}</span>
                  </a>
                ...
                <p className="mx-6 sm:mx-8 mt-5 sm:mt-6 mb-6 sm:mb-8 pt-4 border-t border-slate-800 text-center text-[11px] text-slate-400 font-medium">
                  Activation moyenne : <strong className="text-slate-100">5 minutes</strong>
                </p>
              </div>
            );
          })}
        ...
        <p className="mt-6 text-center text-[11px] sm:text-xs text-slate-500 font-medium">
          Prix TTC · Sans engagement · Paiement sécurisé · Support en français
        </p>
        ...
        <div className="mt-10 sm:mt-14 glass-panel p-5 sm:p-6 rounded-3xl border border-slate-800 max-w-4xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
          <div className="flex items-start sm:items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-400 mt-0.5">
                Écrivez-nous, on trouve le pack qu’il vous faut.
              </p>
            </div>
          </div>
          ...
          <a
            href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
              'Bonjour ! Quel pack me conseillez-vous ?'
            )}`}
            target="_blank"
            rel="noopener noreferrer"
