...
export const Hero: React.FC = () => {
  const whatsappOrderUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Bonjour ! Je souhaite commander un abonnement IPTV SwivTV 4K.')}`;
  ...
  return (
    <section id="hero" className="relative scroll-mt-20 sm:scroll-mt-28 pt-10 pb-14 md:pt-40 md:pb-24 overflow-hidden bg-slate-950">
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(50% 60% at 24% 38%, rgba(0,85,164,0.38), transparent 70%),' +
            'radial-gradient(46% 56% at 76% 34%, rgba(239,65,53,0.30), transparent 70%),' +
            'radial-gradient(40% 50% at 50% 78%, rgba(122,168,224,0.28), transparent 72%)',
          filter: 'blur(28px)'
        }}
      />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-4xl mx-auto space-y-5 sm:space-y-7 rise-in" style={{ animationDelay: '80ms' }}>
          <h1 className="font-display text-[2.15rem] sm:text-6xl md:text-7xl text-slate-100 leading-[0.95] sm:leading-[0.92] text-balance">
            <span className="text-gradient whitespace-nowrap">80&nbsp;000+ chaînes</span>{' '}
            <span className="sm:hidden">en 4K</span>
            <span className="hidden sm:inline">
              et <span className="whitespace-nowrap">200&nbsp;000+ films</span> &amp; séries en{' '}
          ...
          <p className="sm:hidden text-[15px] text-slate-400 leading-relaxed px-2">
            200 000+ films et séries, sur tous vos écrans.
          </p>

          <p className="hidden sm:block text-sm sm:text-xl text-slate-300 max-w-2xl mx-auto font-normal leading-relaxed">
            Ligue 1, Champions League, films et séries en 4K sur tous vos écrans.
          </p>

          {/* Three claims the rest of the page already makes */}
          <div className="grid grid-cols-3 gap-2 sm:gap-5 sm:max-w-2xl sm:mx-auto">
            {[
              { k: '4K', v: '60 FPS', from: '#3d7fcc', to: '#0055a4' },
              { k: '99,9 %', v: 'Dispo', from: '#0055a4', to: '#002f63' },
              { k: '5 min', v: 'Install.', from: '#ef4135', to: '#cf2b20' }
            ].map((stat, i) => (
              <div
                key={stat.k}
                className="relative overflow-hidden rounded-2xl sm:rounded-3xl border border-slate-800 bg-slate-950 px-2 py-3 sm:px-4 sm:py-6 shadow-[0_10px_24px_-18px_rgba(13,20,40,0.5)]"
              >
                <span
                  aria-hidden
          ...
          <div className="pt-1 sm:pt-2 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
            ...
            >
              ...
              <span className="relative">Commander sur WhatsApp</span>
          ...
          <div className="hidden sm:flex pt-3 flex-wrap items-start justify-center gap-y-2 gap-x-6 text-sm text-slate-400 font-medium">
            <span className="flex flex-col sm:flex-row items-center gap-1 sm:gap-1.5 text-center">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Sans engagement
            </span>
            <span className="flex flex-col sm:flex-row items-center gap-1 sm:gap-1.5 text-center">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Activation en 5 minutes
            </span>
            <span className="flex flex-col sm:flex-row items-center gap-1 sm:gap-1.5 text-center">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> Satisfait ou remboursé
            </span>
          </div>
